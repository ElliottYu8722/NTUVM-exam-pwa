...
function loadNotes() {
  // 不再把 notes_v2 整包 parse 到 state.notes（避免 RAM 爆、也避免空白回寫風險）
  state.notes = {};
  state.notesMeta = {};

  // legacy 仍保留在 localStorage（只讀），需要時才載入
  __legacyLoaded = false;
  __legacyNotesObj = null;
  __legacyMetaObj = null;
...
function loadNoteForCurrent() {
  ...
  ensureNoteSeeded(q, scope); // 只補記憶體骨架，不寫入 storage（你原本就這樣設計）[file:2]
  ...
  (async () => {
    try {
      // 若 IDB 沒有但 localStorage 有，做 copy-only 搬移（不會動 legacy）
      await copyLegacyToIdbIfMissing(k);

      const html = await getStoredNoteHtml_NoWrite(k);
      if (typeof html === "string") {
        state.notes[k] = html;
        if (editor.innerHTML !== html) editor.innerHTML = html;
      }

      const meta = await getStoredNoteMeta_NoWrite(k);
      if (meta && typeof meta === "object") {
        state.notesMeta[k] = meta;
      }

      try { await hydrateIdbImagesInEditor(editor); } catch {}
      try { neutralizeOfficeVML(editor); } catch {}
    } catch (e) {
